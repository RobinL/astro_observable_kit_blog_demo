 ...
 * - display() appends into the current cell root (supports multiple calls)
 * - cell roots can be targeted in three ways:
 *   1) By cell id element:    #cell-<id>
 *   2) By output attribute:   [data-cell="<output>"] or targets["<output>"]
 *   3) By output name:        [data-cell="<name>"] or targets["<name>"]
 *
...
export function mount(container = document.body, options = {}) {
  ...
  function cellTargetKeys(cellMeta) {
    const keys = [];
    keys.push("cell-" + cellMeta.id);
    if (cellMeta.output) keys.push(cellMeta.output);
    if (Array.isArray(cellMeta.outputs)) keys.push(...cellMeta.outputs);
    return keys;
  }

  function findTargetedRoot(cellMeta) {
    const keys = cellTargetKeys(cellMeta);

    // 1) Programmatic targets map
    for (const key of keys) {
      const el = targets[key];
      if (el) return el;
    }

    // 2) Declarative DOM targeting via data-cell
    for (const key of keys) {
      const selector = '[data-cell="' + CSS.escape(String(key)) + '"]';
      const el = document.querySelector(selector);
      if (el) return el;
    }

    // 3) Canonical per-cell id
    const existing = document.getElementById("cell-" + cellMeta.id);
    if (existing) return existing;

    // 4) Fallback: create root (optionally append)
    const root = document.createElement("div");
    root.id = "cell-" + cellMeta.id;
    if (appendUnmatched) container.appendChild(root);
    return root;
  }

  // Define each cell with its own root element
  for (const cellMeta of cells) {
    const root = findTargetedRoot(cellMeta);
